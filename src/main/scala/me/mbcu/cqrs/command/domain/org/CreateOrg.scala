...
import me.mbcu.cqrs.command.app.CommandError.{BadDto, BadToken}
...
case class CreateOrg(orgId: UUID, name: Name, location: Location, description: Description) extends Command(orgId) {
...
object CreateOrg extends CommandBuilder {
  ...
  override def build(payload: HttpPayload, crypto: Config.CommandCrypto): Task[Either[CommandError, Command]] =
    Task(
      ...
      } yield CreateOrg(f.jwtPayload.sub, c, d, e)
    )
}
